...
class APIServerAdapter(BasePlatformAdapter):
    ...
    # ------------------------------------------------------------------
    # Ana atendente (Cesto d'Amore) — dedicated inbound endpoint
    # ------------------------------------------------------------------

    @staticmethod
    def _ana_idle_seconds(ts: Any) -> float:
    ...
    async def _handle_ana_message(self, request: "web.Request") -> "web.Response":
        ...
        Body: {sessionKey, pushName, message}. ``sessionKey`` is the
        authoritative per-customer session id (routed exactly as received —
        not derived from phone/chatId). ``pushName`` is display/context only.
        Ana's reply is returned in the HTTP body; n8n delivers it over WhatsApp.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app.router.add_post("/api/ana/message", self._handle_ana_message)
