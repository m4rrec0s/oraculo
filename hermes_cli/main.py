...
def _try_fast_chat_launch() -> bool:
    ...
    if getattr(args, "oneshot", None):
        ...
        # Enterprise: resolve model/provider from config when not explicitly
        # provided (entrypoint seeds config.yaml; CLI flags stay authoritative).
        model = getattr(args, "model", None)
        provider = getattr(args, "provider", None)
        if model is None or provider is None:
            ...
            if isinstance(model_cfg, dict):
                if model is None:
                    model = model_cfg.get("name") or model_cfg.get("default") or model_cfg.get("model")
                if provider is None:
                    ...
                    if provider is None and model:
                        ...
                        detected = detect_provider_for_model(model, current_provider)
                        if detected:
