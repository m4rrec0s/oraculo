...
export const api = {
  ...
  getPersonaPersonas: () =>
    fetchJSON<{ personas: PersonaInfo[] }>("/api/admin/persona-personas"),
  getPersonaSessions: (persona: string, limit = 50, offset = 0) =>
    fetchJSON<PersonaSessionsResponse>(
      `/api/admin/persona-sessions?persona=${encodeURIComponent(persona)}&limit=${limit}&offset=${offset}`,
    ),
  renamePersonaSession: (sessionId: string, title: string) =>
    fetchJSON<ActionResponse>(`/api/admin/persona-sessions/${encodeURIComponent(sessionId)}/rename`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: title }),
    }),
  togglePersonaSession: (sessionId: string) =>
    fetchJSON<ActionResponse>(`/api/admin/persona-sessions/${encodeURIComponent(sessionId)}/toggle`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    }),
  deletePersonaSession: (sessionId: string) =>
    fetchJSON<ActionResponse>(`/api/admin/persona-sessions/${encodeURIComponent(sessionId)}/delete`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    }),
};
